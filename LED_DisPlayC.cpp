#include "LED_DisPlayC.h"

LED_DisPlayC::LED_DisPlayC()
{
}
...
void LED_DisPlayC::begin(uint8_t LEDNumber)
{
...
void LED_DisPlayC::run(void *data)
{
    data = nullptr;
...
void LED_DisPlayC::_displaybuff(uint8_t *buffptr, int32_t offsetx, int32_t offsety)
{
...
void LED_DisPlayC::animation(uint8_t *buffptr, uint8_t amspeed, uint8_t ammode, int64_t amcount)
{
    xSemaphoreTake(_xSemaphore, portMAX_DELAY);
    if (_mode == kAnimation_run)
    {
...
void LED_DisPlayC::displaybuff(uint8_t *buffptr, int32_t offsetx, int32_t offsety)
{
    uint16_t xsize = 0, ysize = 0;
    xsize = _xColumns;
    ysize = _yRows;
...
void LED_DisPlayC::setBrightness(uint8_t brightness)
{
    xSemaphoreTake(_xSemaphore, portMAX_DELAY);
...
void LED_DisPlayC::drawpix(uint8_t xpos, uint8_t ypos, CRGB Color)
{
    if ((xpos >= 5) || (ypos >= 5))
    {
        return;
...
void LED_DisPlayC::drawpix(uint8_t Number, CRGB Color)
{
    if (Number >= NUM_LEDS)
    {
        return;
...
void LED_DisPlayC::fillpix(CRGB Color)
{
    xSemaphoreTake(_xSemaphore, portMAX_DELAY);
...
boolean LED_DisPlayC::animationrunning()
{
    if (_mode == kAnimation_run)
    {
        return true;
...
void LED_DisPlayC::setWidthHeight(uint16_t xColumns, uint16_t yRows)
{
    _xColumns = xColumns;
    _yRows = yRows;
}
